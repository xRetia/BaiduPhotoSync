...
class Album(apiObject):
    ...
    def get_self_1page(cls, req, cursor=None):
        ...
        if isinstance(pageInfo, dict) and pageInfo.get("errno") not in (None, 0):
            raise RuntimeError(
    ...
    def get_sub_1page(self, cursor=None):
        ...
        if isinstance(res, dict) and res.get("errno") not in (None, 0):
            raise RuntimeError(
                "listfile 接口返回错误：errno={}，request_id={}".format(
                    res.get("errno"), res.get("request_id")
                )
            )
        if not isinstance(res, dict) or "list" not in res:
            raise RuntimeError("listfile 接口响应缺少 list 字段：{}".format(res))
        return {
            "items": [
